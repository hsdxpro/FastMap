...
static int RobinMapAccessCounter;
...
class hash_table
{
  static const int INITIAL_SIZE = 256;
	...
	{				
		...
		auto h = key;// static_cast<uint32_t>(hasher(key));
	...
	{	
		//auto myHash = elem_hash(slot_index);

		//int old = (slot_index + capacity - desired_pos(hash)) & mask;
		int asd = (slot_index + capacity - hash & mask);
		return asd;
	}
	...
	int lookup_index(const Key& key) const
	{
		...
		for(;;)
		{						
			//++RobinMapAccessCounter;
			if (elem_hash(pos) == hash && buffer[pos].key == key)
				return pos;
			else if (dist > probe_distance(elem_hash(pos), pos))
				return -1;
			else if (elem_hash(pos) == 0)
				return -1;
